      ...
      if (r_file == 1) then
            open (unit=1,  file='C:\peter_abaqus\Summer-Research-Project\meep\meep_out\v_500_res_75_t_20.bin', 
...
c use code to define DDSDDE, STRESS, STATEV, SSE, SPD, SCD, and if necessary, RPL, DDSDDT, DRPLDE, DRPLDT, PNEWDT
      ...
      coord_conv(1) = ceiling((coords(1)+0.5)*75)
      coord_conv(2) = ceiling((coords(2)+0.5)*75)
      coord_conv(3) = ceiling((coords(3)+0.5)*75)

      
      flux(1) = data_out(15, int(coord_conv(1)), int(coord_conv(2)), int(coord_conv(3)))**2*10e12
