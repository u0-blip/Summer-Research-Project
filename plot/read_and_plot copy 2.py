...
# %%
import pickle
f_name = 'cube_r_0.5_gap_0.0_xloc_0.0_fcen_0.8167_ff_0.99_rt_0.0_fill_factor_0.01_0.99_64eps_rock_1'
dir = '/mnt/c/peter_abaqus/Summer-Research-Project/output/export/3/'
...
absorption = data[:, 0]*eps*loss_factor/fill_factor
...
grad = np.gradient(y_pred, fill_factor)

color1 = 'tab:blue'
color2 = 'tab:red'

plt.plot(fill_factor, y_pred, color=color1)
# plt.plot(np.linspace(0.01, 0.5, 16), y_pred1, color=color1)
plt.plot(fill_factor, absorption, "x", color=color1)
# plt.plot(np.linspace(0.01, 0.5, 16), absorption1, "x", color=color1)


# plt.legend('Fitted curve', 'rate of change')
ax1 = plt.gca()
plt.title('fill factor on EM absorption', fontsize=25)
ax1.set_xlabel('fill factor', fontsize=20)
ax1.set_ylabel('Temp change (°C)', fontsize=20, color=color1)
# plt.legend((line,), fontsize=10)
# plt.legend(('eps = 7, 1', 'eps = 7, 7'))
ax1.tick_params(labelsize=18)
ax1.tick_params(labelsize=18, axis='y', labelcolor=color1)
ax1.set_ylim(bottom=0)


ax2 = ax1.twinx()  # instantiate a second axes that shares the same x-axis
ax2.plot(fill_factor, grad/100, color=color2)
ax2.set_ylabel('ROC (°C/1% fill factor)', fontsize=20, color=color2)  # we already handled the x-label with ax1
ax2.tick_params(labelsize=18, axis='y', labelcolor=color2)
ax2.set_ylim(bottom=0, top=3)

plt.savefig('/mnt/c/peter_abaqus/Summer-Research-Project/output/export/3' + '/sweep_roc1.png', dpi=400, bbox_inches='tight')
