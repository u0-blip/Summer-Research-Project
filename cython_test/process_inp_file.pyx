...
input_f_name = config['process_inp']['input_f_name']
input_geo_name = config['process_inp']['input_geo_name']
...
processed_f_name = input_f_name[:-4] + '_out' + input_f_name[-4:]
...
def processing(input_f_name):
    f = open(abaqus_dir + input_f_name, 'r')
...
if __name__ == '__main__':
    # before = time()
    # voronoi_create(mesh_size=0.005, display=False, ingeo=input_geo_name,out_f=input_f_name)
    # after = time()
