...
from gen_voronoi.simple_geo_and_arg import get_polygon_coord
...
def vis(Simulation):
    Simulation.init_sim()
    eps_data = Simulation.get_epsilon()
...
def gen_part_size(num_crystal, size_crystal_base, weibull = True):
    ...
    for i in range(num_crystal):
        size.append(mp.Vector3(*size_crystal_base[i, :]))
...
def gen_checker(config):
    checker_coord = []
    particle_size = config.getfloat('Geometry', 'particle_area_or_volume')
    radius = get_rad(particle_size, config)
    size_cell = get_array('Geometry', 'cell_size', config)
    ff = config.getfloat('Geometry', 'fill_factor')
    r_sub_cell = np.sqrt((1/ff)*particle_size)
    size_cell -= 2

    size_cell[0]/r_sub_cell
...
def gaussian_size(num, config):
    particle_size = config.getfloat('Geometry', 'particle_area_or_volume')
    std = config.getfloat('Geometry', 'std')
