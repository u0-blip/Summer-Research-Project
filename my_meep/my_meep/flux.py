...
class Flux_manager():
    ...
    def get_flux_region(self):
        ...
        in_sim_frequency = self.config.getfloat('Source', 'fcen')
        flux_width = self.config.getfloat('Source', 'flux_width')
        # the following side are added in a clockwise fashion
        side = [self.Simulation.add_flux(in_sim_frequency, flux_width*in_sim_frequency, nfreq, fr) for fr in frs]
        # transmitted flux
        return self.Simulation, side

    def sim_run(self, Simulation):
        out_every = self.config.getfloat('self.Simulation', 'out_every')
        pt = gen_geo_helper.np2mp(get_array('Source', 'near_flux_loc', self.config))
        self.Simulation.run(mp.at_every(out_every, f1),
    ...
    def get_fluxes(self, basic_sim, sides, basic_sides):
        ...
        self.Simulation.load_minus_flux_data(sides[0], basic_trans_flux_data)

        ez_data = self.sim_run(self.Simulation)
        ...
        if mp.am_master():
            ...
            plt.savefig('_'.join([os.getcwd() + '/output/export/3/', self.config.get(
                'Geometry', 'shape'), self.config.get('Geometry', 'particle_area_or_volume')]) + '_flux.png', dpi=300, bbox_inches='tight')
